...
( async () => {
let json;
let responsePosts = await fetch('https://dummyjson.com/posts/' + topicId);
if (responsePosts.ok) {
  json = await responsePosts.json();
} else {
  alert("Ошибка HTTP: " + responsePosts.status);
}
let parsedPosts = json;
//let responseComments = await fetch('https://jsonplaceholder.typicode.com/comments/post/' + topicId);
//if (responseComments.ok) {
//  json = await responseComments.json();
//} else {
//  alert("Ошибка HTTP: " + responseComments.status);
//}
//let parsedComments = json;
...
topicDiv.innerHTML += `<h4 class="topicTitle">${parsedPosts.title}</h4>`;
topicDiv.innerHTML += `<p class="topicText">${parsedPosts.body}</p>`;
